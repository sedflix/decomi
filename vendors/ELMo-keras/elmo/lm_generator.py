...
class LMDataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, corpus, vocab, sentence_maxlen=100, token_maxlen=50, batch_size=32, shuffle=True,
                 token_encoding='word', for_MTL=False, is_test=False):
    ...
    def __getitem__(self, index):
        ...
        if self.for_MTL:
            return [word_indices_batch, for_word_indices_batch]
        ...
        if self.is_test:
            return [word_indices_batch if self.token_encoding == 'word' else word_char_indices_batch,
                    for_word_indices_batch, back_word_indices_batch]
        else:
            return [word_indices_batch if self.token_encoding == 'word' else word_char_indices_batch,
                    for_word_indices_batch, back_word_indices_batch], []
    ...
    def get_token_indices(self, sent_id: int):
        ...
        j = 0
        for j, token in enumerate(self.lines[sent_id].split()[:self.sentence_maxlen - 2]):
...
class MTLLMDataGenerator(keras.utils.Sequence):
    """
    ...
    def __len__(self):
        """Denotes the number of batches per epoch"""
        return int(np.ceil(len(self.data3.indices) / self.batch_size))
    ...
    def __init__(self, corpus, vocab, sentence_maxlen=100, token_maxlen=50, batch_size=32, shuffle=True,
        ...
        """
        self.batch_size = batch_size
        self.data1 = LMDataGenerator(corpus[0], vocab, sentence_maxlen, token_maxlen, batch_size, shuffle,
                                     token_encoding, for_MTL=True)
        self.data2 = LMDataGenerator(corpus[0], vocab, sentence_maxlen, token_maxlen, batch_size, shuffle,
                                     token_encoding, for_MTL=True)
        self.data3 = LMDataGenerator(corpus[0], vocab, sentence_maxlen, token_maxlen, batch_size, shuffle,
                                     token_encoding, for_MTL=True)

    def __getitem__(self, index):
        arr = self.data1.__getitem__(index)
        arr.extend(self.data2.__getitem__(index))
        arr.extend(self.data3.__getitem__(index))
        return arr, []

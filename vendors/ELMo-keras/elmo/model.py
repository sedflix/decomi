...
class ELMo(object):
    def __init__(self, parameters):
        ...
        os.makedirs(os.path.join(MODELS_DIR, parameters['name']), exist_ok=True)
        self.compile_elmo()
    ...
    def compile_elmo(self, print_summary=False):
        ...
        if self.parameters['unidirectional'] is False:
    ...
    def train(self, train_data, valid_data):
        ...
        weights_file = os.path.join(MODELS_DIR, self.parameters['name'], "elmo_best_weights.hdf5")
    ...
    def evaluate_memory_efficient(self, test_data):
        ...
        for i in range(len(test_data)):
            ...
            x = np.asarray(test_batch[0])
            y_true_forward = np.asarray(test_batch[1])
            y_true_backward = np.asarray(test_batch[2])
            y_pred_forward = self._model.predict([x, y_true_forward, y_true_backward])
            y_true_forward, y_pred_forward = unpad(x, y_true_forward, y_pred_forward)
            perplexity.append(ELMo.perplexity(y_pred_forward, y_true_forward))
    ...
    def wrap_multi_elmo_encoder(self, print_summary=False, save=False):
        ...
        if save:
            self._elmo_model.save(os.path.join(MODELS_DIR, self.parameters['name'], 'ELMo_Encoder.hd5'))
    ...
    def generate(self, seed, size=32):
        for i in range(len(seed), size):
            ...
            pred = self._model.predict([x_seed, temp_seed_forward, temp_seed_forward])
